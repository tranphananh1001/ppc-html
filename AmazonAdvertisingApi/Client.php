...
class Client {
    ...
    private function _download($location, $gunzip = false) {
        ...
        if($gunzip)
            $request->setOption(CURLOPT_ENCODING, 'gzip');
        $request->setOption(CURLOPT_HTTPHEADER, $headers);
        $request->setOption(CURLOPT_USERAGENT, $this->userAgent);
    ...
    private function _operation($interface, $params = array(), $method = "GET", $refresh_token = true) {
        $headers = array(
        ...
        if($response['code'] == 401 AND $refresh_token) {
            $this->doRefreshToken();
            return $this->_operation($interface, $params, $method, false);
        }
        else
            return $response;
    }
